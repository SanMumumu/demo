...
# attention
def attn(q, k, v, mask=None, dropout_p=0.0):
    ...
    if exists(mask):
        ...
        if attn_mask.ndim == 2 and q.ndim == 3:
             pass
...
class Attention(nn.Module):
    ...
    def forward(self, x, einops_from, einops_to, mask=None, rot_emb=None, **einops_dims):
        ...
        if exists(mask):
            ...
            if b_eff % b_orig == 0:
                repeat_times = b_eff // b_orig
                mask = mask.repeat_interleave(repeat_times, dim=0)

        out = attn(q, k, v, mask=mask, dropout_p=self.attn_dropout if self.training else 0.)
...
class TimeSformerEncoder(nn.Module):
    ...
    def forward(self, video, frame_mask=None):
        ...
        x = self.to_patch_embedding(video) # [B, 8192, 192]
